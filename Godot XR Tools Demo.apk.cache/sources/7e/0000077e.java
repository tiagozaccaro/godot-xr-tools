...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1473plusAssignLRDsOJo(long duration) {
        long newReading;
        long longDelta = Duration.m1385toLongimpl(duration, getUnit());
        if (longDelta != Long.MIN_VALUE && longDelta != LongCompanionObject.MAX_VALUE) {
            long j = this.reading;
            newReading = j + longDelta;
            if ((j ^ longDelta) >= 0 && (j ^ newReading) < 0) {
                m1472overflowLRDsOJo(duration);
            }
        } else {
            double delta = Duration.m1382toDoubleimpl(duration, getUnit());
    ...
    private final void m1472overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + "ns is advanced by " + ((Object) Duration.m1388toStringimpl(duration)) + '.');
    }
}